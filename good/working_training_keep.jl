...
loader_test = DataLoader((data=X[:, test_idx], label=Y[:, test_idx]), batchsize=1024, shuffle=true);

model = Chain(Dense(2, hidden_dim, relu),
              Dense(hidden_dim, hidden_dim, relu),
              Dense(hidden_dim, hidden_dim, relu),
              Parallel(vcat, x -> x, Chain(Dense(hidden_dim, num_classes, bias=false), x -> softmax(x))))
...
for epoch in 1:num_epochs
    for (x,y) in loader_train
        ...
        loss, back = Zygote.pullback(ps) do 
            ...
            tmp = A * A';
            loss_triu = 2f0 / (batch_size * (batch_size - 1)) * sum([i > j ? tmp[i, j] : 0.0 for i ∈ 1:num_classes, j ∈ 1:num_classes])
